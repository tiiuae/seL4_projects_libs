...
struct clock_data clock_data[] = {
    ...
    [CLK_SCLKMPLL] = CLK_DEFN_NONE(),
    [CLK_SCLKBPLL] = CLK_DEFN_NONE(),
    [CLK_SCLKCPLL] = CLK_DEFN_NONE(),
    [CLK_SCLKGPLL] = CLK_DEFN_NONE(),
    [CLK_SCLKEPLL] = CLK_DEFN_NONE(),
    [CLK_SCLKVPLL] = CLK_DEFN_NONE(),
...
vm_clock_config(struct clock_device* clkd, enum clk_id clk_id, int grant)
{
    ...
    for (i = 0; i < data->nregs; i++) {
        ...
        mask_val = MASK(data->regs[i].bits) <<  data->regs[i].shift;
        if (grant) {
            *mask |= mask_val;
        } else {
            *mask &= ~mask_val;
