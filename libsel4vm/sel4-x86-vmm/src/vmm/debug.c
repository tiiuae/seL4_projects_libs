...
void vmm_print_guest_context(int level, vm_vcpu_t *vcpu) {
    ...
    DPRINTF(level, "exit info : reason 0x%x    qualification 0x%x   instruction len 0x%x interrupt info 0x%x interrupt error 0x%x\n",
                    vmm_guest_exit_get_reason(&vcpu->vcpu_arch.guest_state), vmm_guest_exit_get_qualification(&vcpu->vcpu_arch.guest_state), vmm_guest_exit_get_int_len(&vcpu->vcpu_arch.guest_state), vmm_vmcs_read(vcpu->vm_vcpu.cptr, VMX_DATA_EXIT_INTERRUPT_INFO), vmm_vmcs_read(vcpu->vm_vcpu.cptr, VMX_DATA_EXIT_INTERRUPT_ERROR));
    DPRINTF(level, "            guest physical 0x%x     rflags 0x%x \n",
                   vmm_guest_exit_get_physical(&vcpu->vcpu_arch.guest_state), vmm_guest_state_get_rflags(&vcpu->vcpu_arch.guest_state, vcpu->vm_vcpu.cptr));
    DPRINTF(level, "            guest interruptibility 0x%x   control entry 0x%x\n",
                   vmm_guest_state_get_interruptibility(&vcpu->vcpu_arch.guest_state, vcpu->vm_vcpu.cptr), vmm_guest_state_get_control_entry(&vcpu->vcpu_arch.guest_state));
    ...
    DPRINTF(level, "cr0 0x%x      cr3 0x%x   cr4 0x%x\n", vmm_guest_state_get_cr0(&vcpu->vcpu_arch.guest_state, vcpu->vm_vcpu.cptr), vmm_guest_state_get_cr3(&vcpu->vcpu_arch.guest_state, vcpu->vm_vcpu.cptr), vmm_guest_state_get_cr4(&vcpu->vcpu_arch.guest_state, vcpu->vm_vcpu.cptr));
}
