...
static void guest_ram_remove_region(vm_mem_t *guest_memory, int region) {
    ...
    guest_memory->num_ram_regions--;
    memcpy(&guest_memory->ram_regions[region], &guest_memory->ram_regions[region + 1], sizeof(vm_ram_region_t) * (guest_memory->num_ram_regions - region));
    /* realloc it smaller */
...
static int expand_guest_ram_region(vm_mem_t *guest_memory, uintptr_t start, size_t bytes) {
...
static int vmm_alloc_guest_ram_one_to_one(vm_t *vm, size_t bytes) {
    int ret;
    int i;
    ...
    for (i = 0; i < num_pages; i++) {
        ret = vka_alloc_frame(vm->vka, page_size, &objects[i]);
        if (ret) {
    ...
    for (i = 0; i < num_pages; i++) {
        ...
        uintptr_t paddr = vka_object_paddr(vm->vka, &objects[i]);
        if (paddr == VKA_NO_PADDR) {
            ZF_LOGE("Allocated frame has no physical address");
            return -1;
        }
        reservation_t reservation = vspace_reserve_range_at(&guest_memory->vm_vspace, (void*)paddr, BIT(page_size), seL4_AllRights, 1);
        if (!reservation.res) {
            ZF_LOGI("Failed to reserve address 0x%x in guest vspace. Skipping frame and leaking memory!", (unsigned int)paddr);
            continue;
        }
        /* Map in */
        ret = vspace_map_pages_at_vaddr(&guest_memory->vm_vspace, &objects[i].cptr, NULL, (void*)paddr, 1, page_size, reservation);
        if (ret) {
            ZF_LOGE("Failed to map page %d/%d into guest vspace at 0x%x\n", i, num_pages, (unsigned int)paddr);
            return -1;
        }
        /* Free the reservation */
        vspace_free_reservation(&guest_memory->vm_vspace, reservation);
...
int vmm_alloc_guest_device_at(vm_t *vm, uintptr_t start, size_t bytes) {
    ...
    for (i = 0; i < num_pages; i++) {
        reservation_t reservation = vspace_reserve_range_at(&guest_memory->vm_vspace, (void*)(page_start + i * BIT(page_size)), 1, seL4_AllRights, 1);
        if (!reservation.res) {
        ...
        ret = vspace_new_pages_at_vaddr(&guest_memory->vm_vspace, (void*)(page_start + i * BIT(page_size)), 1, page_size, reservation);
        if (ret) {
...
static int vmm_map_guest_device_reservation(vm_t *vm, uintptr_t paddr, size_t bytes, reservation_t reservation, uintptr_t map_base) {
    int page_size = vm->mem.page_size;
    int error;
    vm_mem_t *guest_memory = &vm->mem;
    ...
    for (current_paddr = paddr, current_map = map_base; current_paddr < paddr + bytes;
        ...
        error = simple_get_frame_cap(vm->simple, (void*)current_paddr, page_size, &path);
        uintptr_t cookie;
        bool allocated = false;
        if (error) {
            /* attempt to allocate */
            allocated = true;
            error = vka_utspace_alloc_at(vm->vka, &path, kobject_get_type(KOBJECT_FRAME, page_size), page_size, current_paddr, &cookie);
        }
        if (error) {
            ...
            vka_cspace_free(vm->vka, path.capPtr);
            return error;
        }
        error = vspace_map_pages_at_vaddr(&guest_memory->vm_vspace, &path.capPtr, NULL, (void*)current_map, 1, page_size, reservation);
        if (error) {
            ...
            if (allocated) {
                vka_utspace_free(vm->vka, kobject_get_type(KOBJECT_FRAME, page_size), page_size, cookie);
            }
            vka_cnode_delete(&path);
            vka_cspace_free(vm->vka, path.capPtr);
            return error;
...
uintptr_t vmm_map_guest_device(vm_t *vm, uintptr_t paddr, size_t bytes, size_t align) {
    ...
    reservation_t reservation = vspace_reserve_range(&guest_memory->vm_vspace, bytes + align, seL4_AllRights, 1, (void**)&reservation_base);
    if (!reservation.res) {
    ...
    error = vmm_map_guest_device_reservation(vm, paddr, bytes, reservation, map_base);
    vspace_free_reservation(&guest_memory->vm_vspace, reservation);
    if (error) {
        return 0;
    }
...
int vmm_map_guest_device_at(vm_t *vm, uintptr_t vaddr, uintptr_t paddr, size_t bytes) {
    ...
    /* Reserve a region that is guaranteed to have an aligned region in it */
    vm_mem_t *guest_memory = &vm->mem;
    reservation_t reservation = vspace_reserve_range_at(&guest_memory->vm_vspace, (void*)vaddr, bytes, seL4_AllRights, 1);
    if (!reservation.res) {
        ZF_LOGE("Failed to allocate reservation of size %zu when mapping device from %p at %p", bytes, (void *)paddr,
                (void *)vaddr);
        return -1;
    }
    error = vmm_map_guest_device_reservation(vm, paddr, bytes, reservation, vaddr);
    vspace_free_reservation(&guest_memory->vm_vspace, reservation);
    return error;
}
...
int vmm_alloc_guest_ram_at(vm_t *vm, uintptr_t start, size_t bytes) {
    int ret;
    ret = vmm_alloc_guest_device_at(vm, start, bytes);
    if (ret) {
        return ret;
    }
    ret = expand_guest_ram_region(&vm->mem, start, bytes);
    if (ret) {
        return ret;
    }
...
int vmm_alloc_guest_ram(vm_t *vm, size_t bytes, int onetoone) {
    if (onetoone) {
        return vmm_alloc_guest_ram_one_to_one(vm, bytes);
    }
    ...
    int error;
    int num_pages = ROUND_UP(bytes, BIT(page_size)) >> page_size;
    reservation_t reservation = vspace_reserve_range(&guest_memory->vm_vspace, num_pages * BIT(page_size), seL4_AllRights, 1, (void**)&base);
    if (!reservation.res) {
    ...
    for (int i = 0 ; i < num_pages; i++) {
        ...
        cookie = allocman_utspace_alloc(vm->allocman, page_size, kobject_get_type(KOBJECT_FRAME, page_size), &path, true, &error);
        if (error) {
            ZF_LOGE("Failed to allocate page");
            return -1;
        }
        error = vspace_map_pages_at_vaddr(&guest_memory->vm_vspace, &path.capPtr, &cookie, (void*)(base + i * BIT(page_size)), 1, page_size, reservation);
        if (error) {
            ZF_LOGE("Failed to map page");
            return -1;
        }
    }
    error = expand_guest_ram_region(&vm->mem, base, bytes);
    if (error) {
        return error;
    }
    vspace_free_reservation(&guest_memory->vm_vspace, reservation);
