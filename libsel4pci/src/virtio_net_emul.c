...
static void emul_rx_complete(void *iface, unsigned int num_bufs, void **cookies, unsigned int *lens)
{
    ...
    if (idx != guest_idx) {
        ...
        do {
            ...
            vm_guest_write_mem(emul->emul_vm, buf_base + buf_written, (uintptr_t)desc.addr + desc_written, copy);
            /* update amounts */
            tot_written += copy;
            desc_written += copy;
            buf_written += copy;
...
static void emul_notify_tx(virtio_emul_t *emul)
{
    ...
    while (idx != guest_idx) {
        ...
        do {
            ...
            vm_guest_read_mem(emul->emul_vm, vaddr + len, (uintptr_t)desc.addr + skip, this_len);
            len += this_len;
            desc_idx = desc.next;
