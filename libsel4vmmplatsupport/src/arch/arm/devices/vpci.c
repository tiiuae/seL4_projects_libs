...
int fdt_create_path(void *fdt, const char *path);
...
int fdt_generate_vpci_node(vm_t *vm, vmm_pci_space_t *pci, void *fdt,
                           int gic_phandle)
{
    int err = fdt_prepopulate_vpci_node(vm, fdt);
    if (err) {
        ZF_LOGE("fdt_prepopulate_vpci_node() failed (%d)", err);
        return -1;
    }

    err = fdt_generate_vpci_interrupt_map(vm, pci, fdt, gic_phandle);
    if (err) {
        ZF_LOGE("fdt_generate_vpci_interrupt_map() failed (%d)", err);
        return -1;
    }
